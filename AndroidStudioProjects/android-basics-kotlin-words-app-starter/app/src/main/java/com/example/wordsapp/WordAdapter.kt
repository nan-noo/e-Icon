...
class WordAdapter(private val letterId: String, context: Context) :
    ...
    override fun onBindViewHolder(holder: WordViewHolder, position: Int) {
        ...
        holder.button.setOnClickListener {
            // val queryUrl: Uri = Uri.parse("${DetailActivity.SEARCH_PREFIX}${item}") // activity 이용시
            val queryUrl: Uri = Uri.parse("${WordListFragment.SEARCH_PREFIX}${item}")

            // 컨텍스트와 활동을 전달하는 대신 URI와 함께 Intent.ACTION_VIEW를 전달합니다.
            // ACTION_VIEW는 URI(이 경우 웹 주소)를 사용하는 일반적인 인텐트
            val intent = Intent(Intent.ACTION_VIEW, queryUrl)
            context.startActivity(intent) // intent를 전달하여 시스템에 다른 앱을 실행하라고 지시
        }
